...
public class MyReporterListener extends MyReporterListenerAdapter {
	...
	protected PrintWriter createWriter(String outdir) throws IOException {
		...
		return new PrintWriter(new BufferedWriter(new FileWriter(new File(outdir, "emailable-sterling-report" + ".html"))));
	}
	...
	protected void generateMethodDetailReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r2 : r.values()) {
				ITestContext testContext = r2.getTestContext();
				if (r.values().size() > 0) {
					m_out.println("<h1 class='header' align='center'>" + "Scripts to review manually" + "</h1>");
				}
	...
	private void resultDetail(IResultMap tests) {
		for (ITestResult result : tests.getAllResults()) {
			...
			String cname = method.getMethodName();
			// m_out.println("<h2 id=\"m" + m_methodIndex + "\">" + cname + ":"
			// + method.getMethodName() + "</h2>");
			m_out.println("<h2 id=\"m" + m_methodIndex + "\">" + cname + "</h2>");
			Set<ITestResult> resultSet = tests.getResults(method);
			generateForResult(result, method, resultSet.size());
			m_out.println("<p class=\"totop\"><a href=\"#summary\"></a></p>");
	...
	private void generateForResult(ITestResult ans, ITestNGMethod method, int resultSetSize) {
		...
		if (hasParameters) {
			...
			m_out.print("<th>Parameters</th>");
			/*
			 * for (int x = 1; x <= parameters.length; x++) { m_out.print("<th>Parameter." +
			 * x + "</th>"); }
			 */
			m_out.println("</tr>");
			m_out.print("<tr class=\"param stripe\">");
			/*
			 * for (Object p : parameters) { m_out.println("<td>" +
			 * Utils.escapeHtml(Utils.toString(p))+ "</td>"); }
			 */
			m_out.println("<td>");
			for (Object p : parameters) {
				m_out.println(Utils.escapeHtml(Utils.toString(p)) + "|");
	...
	public void generateSuiteSummaryReport(List<ISuite> suites) {
		...
		m_out.print("<tr>");
		tableColumnStart("Regression Name");
		tableColumnStart("Scripts<br/>Executed");
		tableColumnStart("Script<br/>Passed");
		tableColumnStart("Script<br/>Skipped");
		tableColumnStart("Script<br/>Failed");
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r : tests.values()) {
				...
				qty_skip += q;
				summaryCell(q, 0);
				q = r.getTestContext().getFailedTests().size();
	...
	private void tableStart(String cssclass, String id) {
		m_out.println("<table width='60%' align='center' cellspacing=\"0\" cellpadding=\"0\""
	...
	protected void startHtml(PrintWriter out) {
		...
		out.println("<head>");
		out.println("<title>Execution Report</title>");
		out.println("<style type=\"text/css\">");
		out.println(".swa-headertop {color:#ffffff;font-size:9mm;font-family:consolas }");
		out.println(".header {font-size:7mm;font-family:consolas }");
		out.println("table {margin-bottom:10px;border-collapse:collapse;empty-cells:show}");
		out.println("td,th {border:1px solid #000000;padding:.25em .5em}");
		...
		out.println(".passedodd td {background-color: #65F265}");
		out.println(".passedeven td {background-color: #3F3}");
		out.println(".skippedodd td {background-color: #CCC}");
		out.println(".skippedodd td {background-color: #DDD}");
		out.println(".failedodd td,.numi_attn {background-color: #ff9999}");
		out.println(".failedeven td,.stripe .numi_attn {background-color: #D00}");
		out.println(".stacktrace {white-space:pre;font-family:monospace}");
		out.println(".totop {font-size:85%;text-align:center;border-bottom:2px solid #000}");
		out.println(
				".top-banner-title-font {height:100px; display:block; background-color:#989aa1; color:#ffffff;font-size:15mm}");
		out.println(".table-banner{background-color:#989aa1;height:100px;}");
		out.println("</style>");
		out.println("</head>");
		out.println("<body>");
		out.println("<div class='swa-header' role='banner'>");
		out.println(
				"<table bgcolor='#6897bb' border=1px width='100%' align='center' cellspacing='0' cellpadding='0' class='banner'>");
		out.println("<tr ><th>");
		out.println("<div align='center' class='swa-header--title'>");
		//out.println("<img align='left' src='file:" + (System.getProperty("user.dir")) + "/test-output/softtek_color.png"
		//		+ "'></img>");
		out.println("<h1 class='swa-headertop'>Execution results</h1>");
		out.println("</br/>");
		out.println("</div>");
		out.println("</th></tr>");
		out.println("</table>");
		...
		out.println("<div class='swa-main-page' >");
		out.println("<h1  class='header'align='center'>Execution Summary</h1>");
	}
	...
	protected void endHtml(PrintWriter out) {
		...
		out.println("</br/>");
		out.println("</br/>");
		out.println("<center> Sterling Report </center>");
		out.println("</footer>");
